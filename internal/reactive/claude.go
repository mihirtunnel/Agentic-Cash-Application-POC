...
func callClaudeRaw(agentName string, systemPrompt string, tools []map[string]interface{}, messages []claudeMessage, maxTokens int, round int, verbose bool) (*claudeResponse, error) {
	...
	if verbose {
		reactiveVerboseRequest(agentName, round, messages)
	}
	...
	if verbose {
		reactiveVerboseResponse(agentName, round, &resp)
	}
...
func runToolLoop(
	...
	verbose bool,
) (submitInput json.RawMessage, agentText string, usage TokenUsage, toolCallCount int, err error) {
...
func reactiveVerboseRequest(agentName string, round int, messages []claudeMessage) {
	fmt.Printf("\n  ╔══════════════════════════════════════════════════════════════\n")
	fmt.Printf("  ║  ► [%s] API CALL #%d — SENDING  (%d message(s) in conversation)\n", agentName, round, len(messages))
	fmt.Printf("  ╠══════════════════════════════════════════════════════════════\n")
	for i, msg := range messages {
		fmt.Printf("  ║\n  ║  [msg %d]  role=%s\n", i+1, msg.Role)
		reactiveVerboseContent(msg.Content)
	}
	fmt.Printf("  ╚══════════════════════════════════════════════════════════════\n")
}

func reactiveVerboseResponse(agentName string, round int, resp *claudeResponse) {
	cacheInfo := ""
...
func reactiveVerboseContent(content interface{}) {
	...
	case []interface{}:
		for _, item := range v {
			...
			case "tool_result":
				...
				reactiveIndented("  ║      ", resultContent)
			default:
				fmt.Printf("  ║    %s\n", truncate(string(b), 200))
			}
		}
...
func reactiveIndented(prefix, text string) {
