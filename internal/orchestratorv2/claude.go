...
func callClaudeRaw(agentName string, systemPrompt string, tools []map[string]interface{}, messages []claudeMessage, maxTokens int, round int, verbose bool) (*claudeResponse, error) {
	...
	if verbose {
		v2VerboseRequest(agentName, round, messages)
	}
	...
	if verbose {
		v2VerboseResponse(agentName, round, &resp)
	}

	return &resp, nil
}

// callClaudeSimple makes a single API call with no tools — used for analyst sub-agents
// that only receive data and return a text/structured opinion.
func callClaudeSimple(agentName string, systemPrompt string, userMessage string, maxTokens int, verbose bool) (string, TokenUsage, error) {
	messages := []claudeMessage{
		{Role: "user", Content: userMessage},
	}

	resp, err := callClaudeRaw(agentName, systemPrompt, nil, messages, maxTokens, 1, verbose)
	if err != nil {
...
func v2VerboseRequest(agentName string, round int, messages []claudeMessage) {
	fmt.Printf("\n  ╔══════════════════════════════════════════════════════════════\n")
	fmt.Printf("  ║  ► [%s] API CALL #%d — SENDING  (%d message(s) in conversation)\n", agentName, round, len(messages))
	fmt.Printf("  ╠══════════════════════════════════════════════════════════════\n")
	for i, msg := range messages {
		fmt.Printf("  ║\n  ║  [msg %d]  role=%s\n", i+1, msg.Role)
		v2VerboseContent(msg.Content)
	}
	fmt.Printf("  ╚══════════════════════════════════════════════════════════════\n")
}

func v2VerboseResponse(agentName string, round int, resp *claudeResponse) {
	cacheInfo := ""
...
func v2VerboseContent(content interface{}) {
...
func v2Indented(prefix, text string) {
