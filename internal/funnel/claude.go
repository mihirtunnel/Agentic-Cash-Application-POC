...
func callClaudeRaw(agentName string, systemPrompt string, tools []map[string]interface{}, messages []claudeMessage, maxTokens int, round int, verbose bool) (*claudeResponse, error) {
	...
	if verbose {
		funnelVerboseRequest(agentName, round, messages)
	}
	...
	if verbose {
		funnelVerboseResponse(agentName, round, &resp)
	}
...
func funnelVerboseRequest(agentName string, round int, messages []claudeMessage) {
	fmt.Printf("\n  ╔══════════════════════════════════════════════════════════════\n")
	fmt.Printf("  ║  ► [%s] API CALL #%d — SENDING  (%d message(s) in conversation)\n", agentName, round, len(messages))
	fmt.Printf("  ╠══════════════════════════════════════════════════════════════\n")
	for i, msg := range messages {
		fmt.Printf("  ║\n  ║  [msg %d]  role=%s\n", i+1, msg.Role)
		funnelVerboseContent(msg.Content)
	}
	fmt.Printf("  ╚══════════════════════════════════════════════════════════════\n")
}

func funnelVerboseResponse(agentName string, round int, resp *claudeResponse) {
	cacheInfo := ""
...
func funnelVerboseContent(content interface{}) {
...
func funnelIndented(prefix, text string) {
