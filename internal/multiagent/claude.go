...
// callClaudeRaw makes a single API call to Claude with the given system prompt, tools, and messages.
// It handles caching headers, request marshaling, and response parsing.
// When verbose is true it prints the full request and response in the same box format as the single agent.
func callClaudeRaw(agentName string, systemPrompt string, tools []map[string]interface{}, messages []claudeMessage, maxTokens int, round int, verbose bool) (*claudeResponse, error) {
	...
	if verbose {
		maVerboseRequest(agentName, round, messages)
	}

	// Block until a rate-limit slot is available (shared across all goroutines/agents).
	ratelimit.Wait()
	...
	if verbose {
		maVerboseResponse(agentName, round, &resp)
	}

	return &resp, nil
}

// runToolLoop executes a multi-round agent loop: sends messages to Claude, executes any tool calls
// locally, appends results, and repeats until the model stops calling tools or the budget is reached.
// The submitHandler is called when a tool named submitToolName is invoked, returning the raw JSON input.
func runToolLoop(
	agentName string,
	systemPrompt string,
	...
	for round := 1; round <= maxRounds; round++ {
		...
		// Execute tools and build the next round.
...
// maVerboseRequest prints the full conversation being sent for this API call, with the agent name
// in the header so you can distinguish NameAgent / AmountAgent / EmailAgent / MergerAgent calls.
func maVerboseRequest(agentName string, round int, messages []claudeMessage) {
	fmt.Printf("\n  ╔══════════════════════════════════════════════════════════════\n")
	fmt.Printf("  ║  ► [%s] API CALL #%d — SENDING  (%d message(s) in conversation)\n", agentName, round, len(messages))
	fmt.Printf("  ╠══════════════════════════════════════════════════════════════\n")
	for i, msg := range messages {
		fmt.Printf("  ║\n  ║  [msg %d]  role=%s\n", i+1, msg.Role)
		maVerboseContent(msg.Content)
	}
	fmt.Printf("  ╚══════════════════════════════════════════════════════════════\n")
}

// maVerboseResponse prints the full Claude response for this API call.
func maVerboseResponse(agentName string, round int, resp *claudeResponse) {
	cacheInfo := ""
...
// maVerboseContent formats a claudeMessage's Content field for verbose output.
// Content can be a plain string, an assistant block array (text + tool_use), or a tool_result array.
func maVerboseContent(content interface{}) {
...
// maIndented prints multi-line text with a consistent prefix, capping long lines at 300 chars.
func maIndented(prefix, text string) {
